...
| 5 | 折返しが説明できる | 800 MHz を入れると 1228.8 − 800 = **428.8 MHz** に見える。**方形波なら H7 = 700.09 MHz が 528.7 MHz に折り返すので、100 MHz の測定だけでも確認できる** |
| 6 | 振幅が dBFS で整合する | 高調波の dBc が入力レベルに追従するかで歪みの出どころが分かる。**2026-09-16 に達成**（下記） |
| 7 | 無入力時のノイズフロアが妥当 | 全 0 でも飽和でもない。**配線ミスはこの 2 つに振れる** |

## 結果（2026-09-16 時点）

**成功条件 1〜6 を達成。** 残るは 7（無入力のノイズフロア）のみ。

| | |
|---|---|
| ビルド | WNS = **+1.014 ns** / WHS = +0.016 ns、failing endpoints 0 / 26016 |
| ドライバ | `ol.rfdc` = `xrfdc.RFdc`（VLNV 2.6） |
| タイル | `adc_tiles[2]` のみ生存。`PLLLockStatus` = 2、`ClockSource` = 1 |
| **fs（実機）** | **1.2288 GSPS**。`BlockStatus` から読んだ値 |
| DMA | 65536 サンプルを 0.2 ms で取得。`gate status` = 0x2（done） |
| 取りこぼし | `IsFIFOFlagsAsserted` = 0（取得の前後とも） |
| **CW の受信** | **100.0125 MHz → bin 5334 に一致** |
| クロックのずれ | **+14.92 ppm**（SG とボードの周波数差） |
| データ形式 | 14 bit を 16 bit の上位に寄せ（値の刻みが 4） |

### 信号源は方形波だった

入力を 10 dB 下げても高調波が dBc で下がらなかった。

| | −1.74 dBFS | −11.81 dBFS | 方形波の理論値 |
|---|---|---|---|
| H3 | −10.97 dBc | −9.34 dBc | **−9.54 dBc** |
| H5 | — | −14.32 dBc | **−13.98 dBc** |
| H2 | — | −31.53 dBc | 0（対称なら無い） |
| H4 | — | −43.91 dBc | 0（同上） |

**ADC の圧縮なら dBc は 20 dB 下がる。下がらなかった。** 奇数次だけが 1/n で
並ぶので、歪みは入力波形そのもの（方形波）である。信号発生器の出力レベルが
指示値で変わらなかったこととも整合する。

**これは成功条件 6 として、正弦波よりも強い証拠になった。** 方形波の高調波列を
理論値どおり ±0.5 dB で再現したということだからである。

### SMA のラベルとスライス番号は逆

Tile 226 slice 0 だけを有効にしたビットストリームで、**信号が乗ったのは
`ADC_B` の SMA**。ADC_A / ADC_C / ADC_D は白色雑音のみだった（2026-09-16 に確認）。
RefMan の A/B の並びとスライス番号の並びは逆である。
**SMA のラベルで設計を書かないこと。**
...
| `pynq/adc_capture.py` | ボード上での取得と FFT 検証。`--load` で保存済み `.npy` の解析だけもできる |
...
| 高調波が大きい | **dBc が入力レベルに追従するかを見る。** 10 dB 下げて dBc が 20 dB 下がれば ADC の圧縮。変わらなければ入力波形そのものの歪み。奇数次だけが 1/n（−9.54 / −13.98 / −16.90 dBc）で並べば方形波 |
