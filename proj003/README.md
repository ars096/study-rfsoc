# proj003 — RF Data Converter（ADC の生データを PYNQ で読む）
...
**ADC の生サンプルを PS へ吸い上げ、既知の CW トーンで数値が正しいことを確認する。**
...
| 無入力 | `max\|x\|` = 92 / 標準偏差 20.87 / ノイズフロア −106.45 dBFS |

### fs が 1228.8 MSPS ちょうどであることの証拠

第 1 ゾーンと第 2 ゾーンで、クロックのずれが ppm で一致した。

| 入力 | 見えた周波数 | ずれ |
|---|---|---|
| 100.0125 MHz（第 1 ゾーン） | 100.013992 MHz | **+14.92 ppm** |
| 800.0 MHz（第 2 ゾーン、折返し） | 428.788074 MHz | **+14.91 ppm** |

**8 倍離れた周波数で ppm が一致する**ということは、fs が 1228.8 MSPS
ちょうどであることと、ずれが真の周波数比誤差であることの両方を意味する。
どちらかが違えば ppm は一致しない。これが成功条件 3 と 5 の同時達成になっている。

この **+14.9 ppm はボードの VCXO の確度そのもの**で、proj004（外部 10 MHz 基準）で
減るべき量である。**proj003 はこれを測る道具を作った**ことになる。

### 無入力でクロックの回り込みが見えた

入力を外した状態で、第 1 ナイキストゾーンに固定周波数のスパーが 2 本ある。

| 周波数 | 強さ | 素性 |
|---|---|---|
| 491.5125 MHz | −75.66 dBFS | **LMX2594 → RFDC の基準クロック（491.52 MHz）** |
| 245.756 MHz | −76.21 dBFS | **LMK04828 → LMX の基準クロック（245.76 MHz）** |

ノイズフロア（−106 dBFS / 18.75 kHz 分解能）に対して約 30 dB。
**分光計としては固定周波数のバーディーになる。** `adc_capture.py` は上位のピークを
既知のクロック（とその高調波・折返し）と突き合わせて素性を表示する。
...
- **測定器ができた。** ADC の生サンプルを PYNQ から取得し、周波数・振幅・折返し・
  クロックのずれ・回り込みを数値で言える状態になった。以降の proj はこれを使える
- **fs = 1228.8 MSPS / 第 1 ナイキストゾーン DC〜614.4 MHz** を実機で確定した
- **+14.9 ppm のクロックのずれ**が測れている。これが proj004 の出発点になる
- 次: **proj004 で外部 10 MHz 基準クロックを入れる。** CLK_IN0 の SMA は実装済みで
  基板改造は不要だが、`xrfclk` が書き込む LMK04828 のレジスタ設定ファイルを
  差し替える必要がある。**外部基準が来ていなくても PLL2 はオンボード VCXO で
  出力を作るので「動いているように見える」**。判定は PLL1 のロックと、
  この ppm が減ることの 2 点で行う
- その先: 4ch 同時取得と MTS（proj005）、PFB / FFT と積分（Phase 4）
