...
"""proj004 — ADC の生サンプルを取得して FFT し、クロックのずれを ppm で出す。

proj003 から持ち込んだ測定器に、**PLL1 の基準入力を選ぶ `--clkin`** を足したもの。
解析の中身は proj003 と同じで、**条件間で比べるのは ppm だけ**である。
...
BLOCK = 0               # **ADC_B** = Tile 226 slice 0（SMA のラベルと逆。proj003 で実測）
BITFILE = "proj004.bit"
...
def main():
    ...
    r = analyse(x, fs_hz, args.tone * 1e6 if args.tone else None, args.window)

    # **1 行にまとめて出す。** 条件を変えて何度も走らせるので、端末を遡らずに
    # 比べられるようにしておく。
    if r and r.get("ppm") is not None:
        log("")
        log(f"RESULT  clkin={args.clkin}  tone={args.tone} MHz  "
