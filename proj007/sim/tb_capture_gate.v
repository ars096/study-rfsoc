`timescale 1ns/1ps
// capture_gate のテストベンチ（proj006 から内容は変えていない）。
...
module tb;
  ...
  capture_gate #(.DATA_W(W)) dut (
    ...
    // proj007 で増えたポート。**即時モード（trig_mode = ctrl[30] = 0）では使わない**ので
    // このテストベンチは proj006 と同じ内容のまま通る。予約発火の試験は
    // sim/tb_pps_capture.v（pps_capture との結合）で行う。
    .trig(1'b0), .trig_mode_o(), .arm_pulse(), .started(),
