...
- タイミング解析は速度グレード `-2` で実行された
- **`make timing-check` で `-1` でも閉じることを実証した**（WNS +6.700961 ns /
  WHS +0.034556 ns、2026-09-16）。速度グレードが `-2` / `-1` のどちらであっても
  proj002 は成立する。**刻印の確認はこの規模の設計には不要**
  - 検証ビルドは `board_part` とボードプリセットを使わない別の設計なので、
    **合否の判定にのみ使う。`-2` の数字との比較はできない**（[`../VERSIONS.md`](../VERSIONS.md)）
