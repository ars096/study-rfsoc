...
"""proj009 — 内部基準と外部 10 MHz 基準を切り替えて ppm を比べる（proj005 から。判定 5）。

**4ch のうち 1 本だけを見る。**外部基準が効いているかの判定は 1ch で足りる。
見る ch は --ch で選ぶ（既定は CHANS の先頭）。
...
def run_one(clkin, args):
    ...
    fs_hz = args.fs * 1e6
    try:
        # **CHANS の全タイルを起動する。**複数 ch のときは axis_combiner が
        # 全 SI の valid を待つので、どれか 1 タイルでも落ちていると
        # 1 ビートも取れない（proj006 の構成）。proj009 の 1ch では 1 タイル。
    ...
    if args.save_prefix:
        ...
        np.save(path, xs)                 # **4ch まとめて残す**
        log(f"saved: {path}  shape={xs.shape}")
    x = xs[args.ch]
