...
# proj010 — RFDC（fs 4096 MSPS・第 2 ナイキスト）→ ギアボックス → spec_core（8192 点 FFT → 電力 → 積分）→ AXI4-Lite → PS
#
# 出力:
#   build/proj010.bit      ビットストリーム
#   build/proj010.hwh      ハードウェアハンドオフ（PYNQ が読む。.bit と同名にする）
#   build/rfdc_params.rpt  RFDC IP が実際に持つ CONFIG 一覧
#   build/xfft_params.rpt  FFT IP（lane_fft）が実際に持つ CONFIG 一覧
#
# ---- proj009（1ch）からの変更点 ----
#
#   1. **capture_gate・取得用 FIFO・AXI DMA・GPIO を外し、spec_core を置く。**
#      ギアボックスの出口（256 bit / 256 MHz）をそのまま spec_core に入れる
#   2. **読み出しは AXI4-Lite だけ。**spec_core は DSP ドメイン（256 MHz）で AXI4-Lite を受け、
#      PS（pl_clk0）との乗り換えは SmartConnect（NUM_CLKS = 2）にさせる。
#      **自作の CDC を書かない**ための選択。HP ポートと pl_clk1 は使わない
#   3. **FFT IP（xfft）を lane_fft の名前で生成する。**spec_core.v の中で 16 個使う。
#      設定は読み返して検証し、**RTL が前提にしているポート名が IP に在るか**も
#      合成に入る前に照合する（名前が違えば合成エラーになるが、その前に原因を言って止める）
#   4. 1ch 固定（ADC_B = Tile 226 / slice 0）。4ch は proj011 以降
#
# RFDC・Clocking Wizard・ギアボックスは proj009 と同一（設定の順序と読み返しも同じ）。
...
#   **nonrealtime**（入力が途切れたら待つ。フレームの境界がずれない。出力側の tready は 1 に固定）
...
set xfft_req [list \
    ...
    CONFIG.complex_mult_type                      use_mults_performance  0 \
    CONFIG.butterfly_type                         use_xtremedsp_slices   0 \
]
...
set want_ports [dict create \
    ...
    m_axis_data_tdata 48 m_axis_data_tuser 16 m_axis_data_tvalid 1 m_axis_data_tready 1 m_axis_data_tlast 1 \
