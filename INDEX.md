...
| [proj007](proj007/) | 2026-09-17 | 1PPS を受けて絶対時刻を貼る。エポックの付与と、PPS 基準の予約時刻（カウンタ値の一致）での取得開始 | **設計・実装**（ビルド前。`make sim` は ALL PASS）。PPS は毎秒のトリガにせず、一度合わせて以後は監視に回す（fs が整数で 1 s = 153,600,000 ビートちょうど）|
