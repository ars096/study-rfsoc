...
| [proj007](proj007/) | 2026-09-17 | 1PPS を受けて絶対時刻を貼る。エポックの付与と、PPS 基準の予約時刻（カウンタ値の一致）での取得開始 | **ビルド成功**（実機待ち）。WNS +0.285 ns、CRITICAL WARNING なし。最悪経路は 4 本とも `clk_pl_1`（DMA 側）で、**AXIS 153.6 MHz は上位 5 に出ない**。PPS は毎秒のトリガにせず、一度合わせて以後は監視に回す（fs が整数で 1 s = 153,600,000 ビートちょうど）。`report_cdc` の Critical 97 件を読み、**実害のある 2 件（`t_age` 比較器のグリッチ）だけを修正** |
