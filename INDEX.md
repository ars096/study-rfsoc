...
| [proj006](proj006/) | 2026-09-17 | 4ch 同時取得。ADC_A/B/C/D を 512 bit の 1 本に束ねて同じビートで取得し、SMA ラベルとタイル/スライスの対応を実測で確定する。**タイル間のサンプルずれが起動ごとに変わるか**を測る | **進行中**（**ビルド成功**。WNS +0.300 ns / BRAM 123 個。実機での取得はこれから）。MTS は成功条件から外した（自己相関では不要／参照デザインが 500 MHz 系で fs を捨てることになる／PL SYSREF の経路が未確認）|
