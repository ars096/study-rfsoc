...
| [proj007](proj007/) | 2026-09-17 | 1PPS を受けて絶対時刻を貼る。エポックの付与と、PPS 基準の予約時刻（カウンタ値の一致）での取得開始 | **判定 1〜3 成功**（判定 4 待ち）。**1PPS 受信・120 秒で残差 0 ビート = +0.0000 ppb**（分解能 0.0543 ppb。proj004 の CW 法 15 ppb より 2 桁以上良い）。**対照実験**: 10 MHz をスペアナ内蔵水晶に差し替えると **−111.33 ppb**（−9.62 ms/日）を検出。測定が本物であることと、ホールドオーバが基準の質に依存することを実証。+2.5 V / 100 µs でそのまま受かり、`pol=0` が真の立ち上がり、`COMP − TRIG = 58.6 ns` はオープンドレインの RC。WNS +0.287 ns / WHS +0.010 ns、失敗エンドポイント 0、CRITICAL WARNING なし。最悪経路は 4 本とも `clk_pl_1`（DMA 側）で、**AXIS 153.6 MHz は上位 5 に出ない**。PPS は毎秒のトリガにせず、一度合わせて以後は監視に回す（fs が整数で 1 s = 153,600,000 ビートちょうど）。`report_cdc` の Critical 97 件を読み、**実害のある 2 件（`t_age` 比較器のグリッチ）だけを修正** |
