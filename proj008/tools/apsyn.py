...
"""proj006 — AnaPico APSYN420 を USBTMC で叩く（試験トーンの供給）。
...
直接 read/write する（pyvisa も AnaPico 純正ソフトも要らない）。
    ...
    python3 apsyn.py --ref ext --ref-freq 10MHz   # **外部基準に切り替える（proj005）**
...
**proj005 では SG の 10 MHz 基準入力を、ボードと同じ外部基準に繋ぐこと。**
...
# （2026-09-17 に proj005 で発見）。ここは adc_capture.py の FS_HZ と必ず一致させる。
...
def report_reference(ref, want_ext_hz=None):
    """基準クロックの状態を読んで、proj005 として成立しているかを言う。"""
...
def main():
    ...
    p.add_argument("--ref", choices=("int", "ext"), default=None,
                   help="基準クロックを内部／外部に切り替える。"
                        "**proj005 では ext にしてボードと同じ 10 MHz を入れる**")
    ...
    p.add_argument("--preset", choices=sorted(PRESETS),
                   help="proj005 の成功条件に対応する設定を一発で出す")
