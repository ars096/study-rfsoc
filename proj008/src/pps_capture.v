...
module pps_capture #(
    ...
    localparam [31:0] MAGIC = 32'h0008_0001;   // proj008 / rev 1（proj007 rev2 から RTL は同一。番号だけ進める）
    ...
    // データも波形もフラグも正常に見える。
    //
    // **注意: 2026-09-17 に「start_tiles() がこれを起こした」と診断したのは
    // 誤りだった。** rev2 を焼いて epoch を読むと、タイルを起動しても
    // epoch は 1 のままで、リセットは Overlay 直後の 1 回きりだった。
    // `pps_delay.py` が落ちたのは単に「早すぎる判定」である。
    // **したがってこのカウンタは予防であって、観測された事象への対処ではない。**
    //
