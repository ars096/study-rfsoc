...
"""proj008 — ADC 4ch の生サンプルを同時に取得して FFT する（proj007 から変更なし）。
...
CTRL_ARM = 1 << 31
CTRL_TRIG_MODE = 1 << 30        # proj007: 0 = 即時 / 1 = 予約時刻で発火
STATUS_BUSY = 1 << 0
STATUS_DONE = 1 << 1
STATUS_ARMED = 1 << 2           # proj007: 予約を待っている
...
# ------------------------------------------------------------------- 取得
def capture(ol, n_samples, blocks=None, start_at=None, pps=None):
    ...
    **この並びは build.tcl の chans の順序に依存している。**片方を変えたら両方直すこと。
    ...
    start_at を渡すと **予約発火**になる（proj007）。ビートカウンタが start_at に
    達した瞬間に取り始める。`pps` には pps.PPS のインスタンスを渡すこと。
    **PPS を毎秒のトリガにしない**のがこの設計の要で、PPS は start_at を決める
    基準点でしかない（理由は ../README.md）。
    """
    ...
    # proj007: 予約発火。**start_at は arm より前に書く**（データ＋ハンドシェイク）
    mode = 0
    if start_at is not None:
        if pps is None:
            raise ValueError("start_at を使うなら pps.PPS のインスタンスを渡すこと")
        pps.write_start_at(start_at)
        mode = CTRL_TRIG_MODE
    try:
    ...
    gpio.channel1.write(n_beats | mode, 0xFFFFFFFF)     # arm は 0 のまま
    ...
    gpio.channel1.write(n_beats | mode | CTRL_ARM, 0xFFFFFFFF)

    # **late はここで捕まえる。**放っておくと「DMA が返ってこない」という
    # 症状になり、原因が遠くなる（2^32 ビート = 28 秒待つことになる）
    if mode and (pps.flags() & 0x2):
        gpio.channel1.write(n_beats | mode, 0xFFFFFFFF)     # arm を落として予約を取り消す
        raise RuntimeError(
            f"late: 予約した start_at={start_at} は既に過ぎていた。"
            " next_start() の k を増やすか、PS の負荷を下げる")
    ...
    try:
        ...
        if mode:
            log("  **予約発火モードである。**start_at に達していない可能性がある。")
            log("  pps.py --probe で alive と pps_stamp を見る。"
                "PPS が止まれば予約は永久に来ない")
    ...
    gpio.channel1.write(n_beats | mode, 0xFFFFFFFF)     # arm を落とす
    st = gpio.channel2.read()
