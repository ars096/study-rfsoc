    ...
    if len(per_epoch) < 2:
        ...
        if len(cx_means) >= 2:
            ...
            if varies:
                ...
                log("    1 本の PPS では別タイルを較正できない。")
                log("    **構成 B は「タイルごとに PPS 1 本」= 科学 2ch になる。**")
            else:
                log("  → **タイル間の差はエポックをまたいで一定である**（この試行数の範囲で）。")
                log("    一度測れば定数として持てるので、**PPS 1 本（科学 3ch）で足りる。**")
                log("    **ただし「有意差なし」は「差がない」ではない。**")
                log(f"    上の幅 {span * SAMP_NS:.3f} ns を、要求する時刻精度と")
                log("    突き合わせて判断すること")
        else:
