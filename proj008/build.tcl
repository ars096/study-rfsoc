...
# **RTL とブロックデザインは proj007 rev2 と同一。** 変えたのは MAGIC の番号だけ。
# proj008 の中身は RF の配り方（1PPS を 4ch 全部へ）と PS 側のソフト層にある。
# RFDC 4ch → axis_combiner → capture_gate → AXI4-Stream Data FIFO → AXI DMA → PS
#                                  ↑ trig
#                            pps_capture ← PPS Clk（SMA）
#
# 出力:
#   build/proj008.bit      ビットストリーム
#   build/proj008.hwh      ハードウェアハンドオフ（PYNQ が読む。.bit と同名にする）
#   build/rfdc_params.rpt  RFDC IP が実際に持つ CONFIG 一覧（版がズレたときの突き合わせ用）
#   build/comb_params.rpt  axis_combiner の CONFIG 一覧（同上）
#
# ---- proj006 からの変更点は 3 つだけ ----
#
#   1. **pps_capture を足す。** 64 bit のビートカウンタと PPS のタイムスタンプ。
#      **1 秒 = 153,600,000 ビートちょうど**（fs が整数だから成立する）。
#      PPS 入力は 1 本の SMA（`PPS Clk`）から 2 系統が PL へ出ており、
#      IRIG_TRIG_OUT (AH13) と IRIG_COMP_OUT (AJ13) の **両方に**スタンプを打つ
#   2. **capture_gate に予約発火（trig_mode）を足す。**
#      `beat_count == start_at` で起動する。**PPS を毎秒のトリガにはしない** —
#      毎秒叩くとケーブル長差とコンパレータ遅延が毎回データの時刻に乗る
#   3. **AXI GPIO を 2 個足す**（gpio_time_ctrl / gpio_time_stat）。
#      gpio_capture は触らない。proj006 との diff を読めるまま保つため
#
# **mode 0（即時）の振る舞いは proj006 と完全に同じ。**
# PPS 側が動かなかったときに切り分けの基準点が残るようにしてある。
#
...
# ---- 時刻の基準（proj007）----
# **1 秒あたりのビート数が整数であることが、この設計の土台である。**
# fs = 1,228,800,000 / SPW 8 = 153,600,000 ビート/秒ちょうど。
# 割り切れなければ「一度合わせれば以後ずっと整合する」が成立しない。
set beats_sec   [expr {int(round($fs_gsps * 1e9 / $spw))}]
set blank_beats [expr {$beats_sec / 2}]        ;# 0.5 秒。これより短い間隔はグリッチ
set miss_beats  [expr {$beats_sec * 3 / 2}]    ;# 1.5 秒。これを越えたら PPS 欠落
if {abs($fs_gsps * 1e9 / $spw - $beats_sec) > 1e-6} {
    puts "ERROR: fs / SPW が整数ビート/秒にならない（$fs_gsps GSPS / $spw）。"
    puts "  proj007 の設計はここが整数であることに全面的に依存している"
    exit 1
}
...
# ---- 時刻の GPIO（proj007 で追加）----
# **gpio_capture は触らない。** proj006 との diff が「足した分」だけになる。
#   gpio_time_ctrl ch1 出力 32bit: start_at[31:0]
#   gpio_time_ctrl ch2 出力 32bit: [0] snap / [4:1] sel / [5] pps_pol
#   gpio_time_stat ch1 入力 32bit: sel で選んだ語
#   gpio_time_stat ch2 入力 32bit: [0] alive / [1] late / [2] armed / [3] snap_ack / [4] comp_alive
set gpio_tc [create_bd_cell -type ip -vlnv xilinx.com:ip:axi_gpio gpio_time_ctrl]
set_property -dict [list \
    CONFIG.C_GPIO_WIDTH    {32} \
    CONFIG.C_ALL_OUTPUTS   {1} \
    CONFIG.C_IS_DUAL       {1} \
...
set gpio_ts [create_bd_cell -type ip -vlnv xilinx.com:ip:axi_gpio gpio_time_stat]
set_property -dict [list \
    CONFIG.C_GPIO_WIDTH   {32} \
...
# ---- 1PPS の受信とサンプルカウンタ（proj007 で追加）----
set ppsc [create_bd_cell -type module -reference pps_capture pps_capture_0]
set_property -dict [list \
    CONFIG.BEATS_PER_SEC $beats_sec \
    CONFIG.BLANK_BEATS   $blank_beats \
    CONFIG.MISS_BEATS    $miss_beats \
] $ppsc
puts [format "時刻の基準: %d ビート/秒（1 ビート = %.4f ns = %d サンプル）" \
      $beats_sec [expr {1e9 / ($fs_gsps * 1e9 / $spw)}] $spw]
...
set_property -dict [list CONFIG.NUM_SI {1} CONFIG.NUM_MI {5} CONFIG.NUM_CLKS {1}] $smc_ctrl
...
foreach p [list \
        ...
        gpio_time_ctrl/s_axi_aclk  gpio_time_stat/s_axi_aclk \
        pps_capture_0/ctrl_aclk \
...
set adc_rst [list capture_gate_0/aresetn pps_capture_0/aresetn \
                  axis_comb/aresetn axis_fifo/s_axis_aresetn]
...
# 時刻の制御と読み出し（proj007）
nc gpio_time_ctrl/gpio_io_o   pps_capture_0/start_at
nc gpio_time_ctrl/gpio2_io_o  pps_capture_0/ctrl
nc pps_capture_0/rdata        gpio_time_stat/gpio_io_i
nc pps_capture_0/flags        gpio_time_stat/gpio2_io_i

# capture_gate ↔ pps_capture（すべて AXIS ドメイン）
nc capture_gate_0/trig_mode_o pps_capture_0/trig_mode
nc capture_gate_0/arm_pulse   pps_capture_0/arm_pulse
nc capture_gate_0/started     pps_capture_0/started
nc pps_capture_0/trig         capture_gate_0/trig
...
ic [get_bd_intf_pins smc_ctrl/M03_AXI] [get_bd_intf_pins gpio_time_ctrl/S_AXI]
ic [get_bd_intf_pins smc_ctrl/M04_AXI] [get_bd_intf_pins gpio_time_stat/S_AXI]
...
# ---- 1PPS の外部ポート（proj007）----
# **create_bd_port で作るとポート名がそのまま wrapper の top ポート名になる。**
# make_bd_pins_external は `_0` を足すので、XDC の get_ports と食い違う。
# ピンの配置は src/pps.xdc（AH13 / AJ13）。実装後に読み返して確かめる。
foreach {pname ppin what} [list \
    ...
    connect_bd_net [get_bd_ports $pname] [BP pps_capture_0/$ppin]
...
add_files -fileset constrs_1 -norecurse ./src/pps.xdc
...
# ---- 1PPS のポートが意図した足に出ているか（proj007）----
# **これは診断ではなく合否判定なので、catch の外に置く。**
# ピンを 1 本間違えると「PPS が来ない」という症状になり、原因が基板側にも
# ケーブル側にも見えてしまう。**焼く前に確定させる。**
puts ""
puts "---- 1PPS の入力ポート ----"
foreach {pname want} {pps_trig AH13 pps_comp AJ13} {
    set prt [get_ports -quiet $pname]
    if {[llength $prt] == 0} {
        puts "ERROR: ポート $pname が存在しない。"
        puts "  create_bd_port の名前と src/pps.xdc の get_ports を突き合わせる"
        puts "  （make_bd_pins_external を使うと `_0` が付いて食い違う）"
        exit 1
    }
    ...
    if {$got ne $want} {
        puts "ERROR: $pname の配置が $got。期待 $want（RefMan A6 / Appendix A）"
        exit 1
    }
}
